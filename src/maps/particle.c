...
void Particle_SnowDraw(s_Particle* part)
{
    ...
    {
        u8 room = g_SavegamePtr->mapRoomIdx;
...
#ifdef SH_PC_PORT
    /* temp_s3 == 0 when both modulos above land on 0 (particle at the spawn
     * box corner) — rare, but x86 rem-by-zero faults where MIPS didn't.
     * The natural result there is vz = 0. */
    if (temp_s3 == 0)
    {
        arg1->vz = 0;
        return;
...
void Particle_SoundUpdate(void)
{
    ...
    switch (g_SavegamePtr->mapIdx)
    {
        ...
        case MapIdx_MAP4_S02:
            ...
            {
                Sfx_WithFalloffAndPitchPlay(Sfx_Unk1360, &g_SysWork.playerWork.player.position, unkValDiv4, Q12(0.0f), 0);
            }
