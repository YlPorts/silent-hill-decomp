...
// Re-added from fork game.h (dropped during the upstream merge).
...
static inline void Character_AnimSet(s_SubCharacter* chara, s32 animStatus, s32 keyframeIdx)
{
    ...
    chara->model.anim.time        = keyframeIdx << 12;
    chara->model.anim.keyframeIdx = keyframeIdx;
}

/** Caller must declare `s32 i;` in scope. */
...
#define HAS_MAP(mapIdx) \
    ((((u32*)&g_SavegamePtr->paperMapFlags)[(mapIdx) / 32] >> ((mapIdx) % 32)) & (1 << 0))
